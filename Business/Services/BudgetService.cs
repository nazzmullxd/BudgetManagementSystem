    ...
    public class BudgetService : BaseService, IBudgetService
    {
        ...
        public void SetBudgetGoal(string userId, string categoryId, decimal amount, DateTime startDate, DateTime endDate)
        {
            ...
            var budgetGoal = new BudgetGoal
            {
                ...
                GoalName = $"Budget for {category?.CategoryName ?? "Category"}",
                TargetAmount = amount,
                TargetDate = endDate
            };
        ...
        public bool CheckBudgetExceeded(string userId, string categoryId, DateTime startDate, DateTime endDate)
        {
            ...
            bool exceeded = totalExpenses > budgetGoal.TargetAmount;
            _logger.LogInformation("Budget check for user {UserId}, category {CategoryId}: Exceeded = {Exceeded}, Total Expenses = {TotalExpenses}, Budget = {Budget}", userId, categoryId, exceeded, totalExpenses, budgetGoal.TargetAmount);
            return exceeded;
        }
        ...
        public void CheckAndCreateBudgetAlert(string userId, string categoryId, DateTime startDate, DateTime endDate)
        {
            ...
            if (CheckBudgetExceeded(userId, categoryId, startDate, endDate))
            {
                ...
                if (budgetGoal != null)
                {
                    ...
                    var alertMessage = $"Budget exceeded for category '{category?.CategoryName}'. Budget: {budgetGoal.TargetAmount}, Spent: {totalExpenses}";
                    LogAction(_logger, userId, "Budget Alert Created", alertMessage);
                }
            }
        }
    }
